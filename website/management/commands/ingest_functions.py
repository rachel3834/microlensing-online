...
from website.models import Reference, OnlineResource
from website.models import Picture, Movie, Author, SitePage, File
from website.models import TutorialPage, ConceptPage, InteractiveTool
    ...
    for item in entries:
        try:
            ...
            if entry_type == 'URL':
                ...
                print('Error parsing '+entry_type+' file entry: '+str(line))
                print('Problem with item: '+repr(item))
                exit()
...
def resolve_site_link(params,entry_type):
    ...
    if str(params['table']).lower() == 'sitepage':
        ...
        try:
            entry = InteractiveTool.objects.get(name=params['name'])
